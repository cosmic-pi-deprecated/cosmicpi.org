...
class es_cls_sentmail
{
	...
	public static function es_sentmail_ins($guid = "", $qstring = 0, $source = "", $startdt = "", $enddt = "", $count = "", $preview = "")
	{
		global $wpdb;
		$returnid = 0;
		$prefix = $wpdb->prefix;
		...
		$sSql = $wpdb->prepare("INSERT INTO `".$prefix."es_sentdetails` (`es_sent_guid`, `es_sent_qstring`, `es_sent_source`,
								`es_sent_starttime`, `es_sent_endtime`, `es_sent_count`, `es_sent_preview`) 
								VALUES (%s, %s, %s, %s, %s, %s, %s)", array($guid, $qstring, $source, $startdt, $enddt, $count, $preview));			
		$wpdb->query($sSql);
		return true;
	}
	
	public static function es_sentmail_ups($guid = "")
	{
		global $wpdb;
		$returnid = 0;
		$prefix = $wpdb->prefix;
		$currentdate = date('Y-m-d G:i:s'); 
		$sSql = $wpdb->prepare("UPDATE `".$prefix."es_sentdetails` SET `es_sent_endtime` = %s WHERE es_sent_guid = %s LIMIT 1", array($currentdate, $guid));	
		$wpdb->query($sSql);
		return true;
	}
}
